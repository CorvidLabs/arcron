...
The plan's counter-argument -- that `docs/design/1.0.md:92-99` chose rain because it exercises `SKIP_AHEAD` and pulse proves nothing about #7 alone -- is obsolete, though not for the reason first given here. That reason was: **the gate was written when we were the only user**, and 22 of the 28 live upkeeps now belong to accounts other than the deployer, across seven addresses. All seven are ours. [#236](https://github.com/CorvidLabs/arcron/pull/236) corrected the attribution, and this paragraph originally kept the conclusion with a one-line note that "the point survives without it", which was asserted rather than argued. It does not survive in that form: six outside addresses and six of our own are the difference between adoption evidence and a soak test, and this is a soak test.

What survives is narrower and still enough. The registry spans both catch-up policies and cadences from 1,286 to 224,000 rounds, across upkeeps registered by five agents and a human, and it has run without a struct change for the whole period. That is better evidence than one recurring draw we drive, which is all rain ever was, and it does not depend on a repository arcron no longer controls. **We are still the only user.** The gate should say so, and gate on sustained operation rather than on adoption we do not have.
