...
*Recommendation:* one published, wallet-connecting hub **per product**, each at its own canonical address; rain gets `corvidlabs.xyz/rain/console/`. Restate the rule in `web/README.md` and `docs/console-plan.md` as *one canonical address per product* rather than one address total. The evidence: `rain-stat-tiles.ts`'s own doc comment says "Keeper tiles would lie here: this hub is a different app"; `app.ts`'s `rainSurface` fork exists only to hide keeper facts on `/rain`; and `quarantine.ts:40-48` vouches for exactly one canonical app id per network (the keeper's), so the rain hub gets *no* look-alike protection at the shared address today. Two shells each get a working gate; one shell cannot cleanly guard two ids. `web-keeper/` and `web-govern/` are not hubs — they are never published and `web-keeper` has no wallet dependency by design. Post-split: arcron owns 3 apps and publishes 1; rain owns 1 and publishes 1.
...
*Recommendation:* vendor for the first cut. `npm view @corvidlabs/arcron` returns 404 — it has never been published, ships raw TypeScript (`"main": "./src/index.ts"`, no `types`, `tsconfig` is `noEmit`), and resolves today only inside the Bun workspace. `pyproject.toml:7` sets `package-mode = false`. So the framing of "removing rain is a breaking change to a published 1.0.0 surface needing a major bump" is wrong — there is nothing published to break, and nothing for the new repo to install. Copy ~65 TS lines and ~5 Python helpers, accept the drift, and treat publishing as follow-up work with its own issue rather than a gate in front of the split. (`docs/console-plan.md:437-448` records this hazard already biting once, fixed with `prebundle.exclude` in `web/angular.json:76-80`.)
