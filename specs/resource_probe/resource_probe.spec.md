...
| `reenter` | — | `uint64` | Calls the keeper's `execute` back from inside its own execution, once. Measures whether a target can re-enter Archon and who a nested execution would pay. |
