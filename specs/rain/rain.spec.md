...
|--------|-----------|---------|-------------|
| `update` | — | `void` | Creator-only, and only while `frozen` is 0. Replaces the programs. `UpdateApplication` action. |
| `freeze` | — | `void` | Creator-only, once. Sets `frozen` to 1 for good; from there `update` is refused and nothing can restore it. |
...
| Date | Author | Change |
|------|--------|--------|
| 2026-08-27 | CorvidLabs | `update` and `freeze`, matching the keeper's arrangement. This contract is a demo and it iterates, and without an update path the unit-name gate cost a new app id, an abandoned draw holding a 1.98 ALGO prize, and an upkeep re-pointed by hand. It also holds entrants' money, so an upgradeable version is one whose creator can change the rules under people who have already entered. Both are true, so it gets the same two-stage answer: cheap to iterate now, one-way door to close before the money matters. |
