...
| `web/src/app/core/upkeep.test.ts` | unit (`bun test`) | the TypeScript decoder and `effectiveFee` against the *same* recorded box bytes, so the two decoders cannot drift |
