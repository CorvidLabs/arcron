...
- [ ] G2, by 2026-09-19, waits on [#250](https://github.com/CorvidLabs/arcron/issues/250) F01 (pending stranger payload until Discord 2xx, even if the box is later cancelled), F02 (operator-decision policy drafted 2026-09-05; still open until notifier wording and the 24-hour response budget match), F04 (cancelled box must not abort the scan), F05 (real box pagination; not a flood-latency claim), F14 (fee ceiling on `deploy`/`govern`), and F10/F11 evidence. Then: `git tag mainnet-1`, `fledge run deploy-mainnet -- --with-pulse`, `govern status` and `verify_build` recorded privately, keeper and notifier watching the new id before the first upkeep, Pulse `tick` registered at `fee_cap 0` and executed. Soak claims are chain-backed install/update history plus digest, not app age (F03)
