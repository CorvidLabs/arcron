...
2. An ASA bonus is best effort and the ALGO is not. `cancel` pays out no more bonus than the app actually holds, and skips it entirely when the creator is not opted in, so the ALGO refund and the box deletion cannot be blocked by somebody else's asset settings. `execute` likewise skips a bonus the app cannot send. An asset with a clawback address can be taken back out of the app, and a frozen holding still reports a positive balance while refusing to move, so checking the amount alone is not enough. Both share a transaction with the ALGO, and a frozen creator cannot even escape by opting out, because a frozen account cannot close a holding. Trusting the recorded balance would strand escrow permanently on a contract with no delete path.
