...
5. The MBR collected by `register` is exactly what the box costs the app account. The app's spendable balance therefore always covers the total escrow it holds, and every upkeep can pay out its last execution.
...
15. `execute` sends every stored app arg, in order, as the inner call's app args. The selector and each ARC-4 argument travel in an app arg of their own, which is what an ARC-4 method requires. `register` bounds the count at `MAX_CALL_ARGS`, so `execute`'s fan-out is exhaustive.
...
17. `cancel` returns the unspent asset balance along with the ALGO and the box MBR. If the creator cannot receive the asset it refuses, before refunding anything.
